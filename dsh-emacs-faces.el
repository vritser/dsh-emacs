...
;; Shared face definitions for dsh-emacs, modeled after the
;; themes used by agent-shell (xenodium), pi (badlogic/pi-mono), and
;; opencode (anomalyco).
;;
;; Faces use light/dark color specs or inherit standard Emacs faces.
;; The `dsh-emacs-color-*' variables supply palette defaults when this
;; file is first loaded.  Set them before loading the package, or use
;; `custom-set-faces' to customize faces after loading:
;;
...
;;    ...)
;;
;; Fragment faces live in dsh-emacs-ui.el; the active Markdown renderer
;; defines its faces in dsh-emacs-markdown.el.
;;
...
;;; ---------------------------------------------------------------------------
;;; Color token variables (users may redefine them to retheme)
;;; ---------------------------------------------------------------------------
...
(defface dsh-emacs-modeline-ctx-warn-face
  `((((background light)) :foreground "#b45f06")
    ...
    (t :inherit warning))
  "Context usage at least 50% and below 80%."
  :group 'dsh-emacs-faces)
