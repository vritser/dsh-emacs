;;; dsh-emacs-markdown.el --- Full Markdown rendering for dsh-emacs -*- lexical-binding: t -*-
...
;;
;;   (dsh-emacs-markdown-convert "hello **world**")
;;
...
;;
;;   bold        `**X**' / `__X__'        face `dsh-emacs-markdown-bold'
;;   italic      `*X*'   / `_X_'          face `dsh-emacs-markdown-italic'
;;   strike      `~~X~~'                  face `dsh-emacs-markdown-strikethrough'
;;   header      `# X' .. `###### X'      face `dsh-emacs-markdown-header-1' .. `-6'
;;   inline code `` `X` ``                face `dsh-emacs-markdown-inline-code'
;;   link        `[title](url)'           face `dsh-emacs-markdown-link', keymap opens URL
...
;;
;; All dsh-emacs-markdown-* faces inherit from the conventional faces
...
(defgroup dsh-emacs-markdown nil
  "Render Markdown text into propertized form."
  :group 'text)

(defface dsh-emacs-markdown-bold
  '((t :inherit bold))
  "Face for bold text rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-italic
  '((t :inherit italic))
  "Face for italic text rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-strikethrough
  '((t :strike-through t))
  "Face for strikethrough text rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-inline-code
  '((t :inherit font-lock-doc-markup-face))
  "Face for inline code rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-link
  '((t :inherit link))
  "Face for link titles rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-blockquote
  '((t :inherit font-lock-comment-face))
  "Face for blockquoted text rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-header-1
  '((t :inherit org-level-1))
  "Face for level-1 headers rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-header-2
  '((t :inherit org-level-2))
  "Face for level-2 headers rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-header-3
  '((t :inherit org-level-3))
  "Face for level-3 headers rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-header-4
  '((t :inherit org-level-4))
  "Face for level-4 headers rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-header-5
  '((t :inherit org-level-5))
  "Face for level-5 headers rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-header-6
  '((t :inherit org-level-6))
  "Face for level-6 headers rendered by `dsh-emacs-markdown-convert'."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-table-header
  '((t :inherit bold))
  "Face for table header row content."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-table-border
  '((t :inherit font-lock-comment-face))
  "Face for table borders (pipes and dashes)."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-table-zebra
  '((t :inherit default))
  "Face for alternating (zebra) data rows in tables (no background)."
  :group 'dsh-emacs-markdown)

(defface dsh-emacs-markdown-source-block
...
(defface dsh-emacs-markdown-source-block-language
  '((t :inherit (italic font-lock-type-face dsh-emacs-markdown-source-block)))
  "Face for the language label shown above a fenced source block."
  :group 'dsh-emacs-markdown)

(defvar dsh-emacs-markdown-image-max-width 0.4
...
(cl-defun dsh-emacs-markdown-convert (markdown)
...
characters are removed.  See `dsh-emacs-markdown-replace-markup' for
...
`dsh-emacs-markdown-bold' layered on top of any existing face
...
world.\" with face `dsh-emacs-markdown-bold' on \"world\"."
  (let ((case-fold-search nil)
    ...
    (while (re-search-forward
      ...
      (let* ((markup-start (match-beginning 1))
             ...
             (avoid (dsh-emacs-markdown--in-avoid-range-p
...
(cl-defun dsh-emacs-markdown--replace-italics (&key avoid-ranges)
  "Replace `*X*' / `_X_' spans in current buffer with italic X.

Markup characters are deleted; remaining inner text carries face
`dsh-emacs-markdown-italic' layered on top of any existing face
properties.  Spans that fall inside any of AVOID-RANGES are left
untouched.  Returns non-nil if at least one replacement was made.

For example, the buffer \"hello *world*.\" becomes \"hello
world.\" with face `dsh-emacs-markdown-italic' on \"world\"."
  (let ((case-fold-search nil)
        (changed nil))
    (goto-char (point-min))
    (while (re-search-forward
...
(cl-defun dsh-emacs-markdown--replace-strikethroughs (&key avoid-ranges)
  "Replace `~~X~~' spans in current buffer with strike-through-faced X.

Markup characters are deleted; remaining inner text carries face
`dsh-emacs-markdown-strikethrough' layered on top of any existing face
...
`dsh-emacs-markdown-strikethrough' on \"b\"."
  (let ((case-fold-search nil)
        (changed nil))
    (goto-char (point-min))
    (while (re-search-forward
...
(cl-defun dsh-emacs-markdown--replace-headers (&key avoid-ranges)
  "Replace `# X' / `## X' / ... headers with X faced as `org-level-N'.

The `#' prefix and one or more separator spaces are stripped; the
title text is left with face `dsh-emacs-markdown-header-N' where N is
...
with face `dsh-emacs-markdown-header-2' on \"My title\"."
  (let ((case-fold-search nil))
    (goto-char (point-min))
    (while (re-search-forward
      ...
      (let* ((markup-start (match-beginning 0))
        ...
        (if avoid
          ...
          (let* ((level (- (match-end 1) (match-beginning 1)))
                 ...
                 (carried (dsh-emacs-markdown--carry-properties
                           (1- markup-end))))
            (delete-region markup-start markup-end)
            (goto-char markup-start)
            (insert text "\n")
            ...
            (add-face-text-property markup-start
                                    (+ markup-start (length text))
                                    (intern (format "dsh-emacs-markdown-header-%d"
                                                    (min (max level 1) 6))))))))))

(cl-defun dsh-emacs-markdown--style-inline-code (&key avoid-ranges)
  "Strip backticks from complete inline `X` spans and face the body.

The body of each well-formed `` `X` `` is left in place with
face `dsh-emacs-markdown-inline-code' and tagged with the text
property `dsh-emacs-markdown-frozen t' so it is never re-processed
...
face `dsh-emacs-markdown-inline-code' on \"code\"."
  (let ((case-fold-search nil))
    (goto-char (point-min))
    (while (re-search-forward "`\\([^`\n]+\\)`" nil t)
      (let* ((markup-start (match-beginning 0))
...
becomes \"see docs\" with face `dsh-emacs-markdown-link' on \"docs\"
    ...
    (while (re-search-forward
      ...
      (let* ((markup-start (match-beginning 0))
             ...
             (avoid (unless is-image
                      (dsh-emacs-markdown--in-avoid-range-p
                       markup-start markup-end avoid-ranges))))
        (cond
         ...
         (t
          (let ((title (buffer-substring (match-beginning 1) (match-end 1)))
            ...
            (let ((end (+ markup-start (length title))))
              (add-face-text-property markup-start end 'dsh-emacs-markdown-link)
              (put-text-property markup-start end 'keymap
                                 (dsh-emacs-markdown--make-ret-binding-map
                                  (lambda () (interactive)
                                    (dsh-emacs-markdown--open-link url))))
              (put-text-property markup-start end 'mouse-face 'highlight)))))))))

(cl-defun dsh-emacs-markdown--replace-images (&key avoid-ranges)
        ...
        (cond
         ...
         (t
          (let* ((alt (buffer-substring-no-properties
                 ...
                 (url (buffer-substring-no-properties
                       (match-beginning 2) (match-end 2)))
                 (path (dsh-emacs-markdown--resolve-image-url url)))
            (when (and path
...
(cl-defun dsh-emacs-markdown--replace-image-file-paths (&key avoid-ranges)
        ...
        (cond
         ...
         (t
          (let* ((path-start (match-beginning 1))
                 ...
                 (resolved (dsh-emacs-markdown--resolve-image-url raw)))
            (when (and resolved
...
(cl-defun dsh-emacs-markdown--style-dividers (&key avoid-ranges)
...
`dsh-emacs-markdown-frozen' tag so subsequent calls don't re-process
          ...
          (add-text-properties
           ...
           (list 'display
                 (concat (propertize (make-string 12 ?\s)
                         ...
                         "\n")
                 'dsh-emacs-markdown-frozen t
                 'rear-nonsticky '(display dsh-emacs-markdown-frozen))))))))

(cl-defun dsh-emacs-markdown--style-blockquotes (&key avoid-ranges)
...
gets face `dsh-emacs-markdown-blockquote' (composes with any
...
left untouched."
  (let ((case-fold-search nil)
        (bar (propertize "▌" 'face 'dsh-emacs-markdown-blockquote)))
    (goto-char (point-min))
    (while (re-search-forward
            (rx bol (zero-or-more blank)
                ">" (zero-or-more (any " \t>"))
...
major mode (when loadable) and a `dsh-emacs-markdown-frozen t' text
...
(defconst dsh-emacs-markdown--table-pending-line-regexp
...
(defconst dsh-emacs-markdown--table-separator-regexp
  (rx line-start
      (zero-or-more (any " \t"))
      "|"
      (one-or-more (or "-" ":" "|" " " "\t"))
...
(cl-defun dsh-emacs-markdown--find-tables (&key avoid-ranges)
...
Two flavours of region are collected:
  ...
  - Rendered table + extension: a previously-rendered table
    ...
    `dsh-emacs-markdown-table-source' property.  Chars immediately
    ...
    (save-excursion
      (while (< pos (point-max))
        ...
        (cond
         ...
         ((let ((avoid (dsh-emacs-markdown--in-avoid-range-p
                        pos (1+ pos) avoid-ranges)))
            (when avoid (setq pos (cdr avoid)) t)))
         ((get-text-property pos 'dsh-emacs-markdown-table-source)
          (let* ((stashed (get-text-property pos 'dsh-emacs-markdown-table-source))
                 (rendered-end (or (next-single-property-change
                                    pos 'dsh-emacs-markdown-table-source
                                    nil (point-max))
                                   (point-max)))
            ...
            (save-excursion
              ...
              (when (and (< (point) (point-max))
                ...
                (forward-char 1)
                (while (and (not (eobp))
                            (looking-at dsh-emacs-markdown--table-line-regexp)
                            (not (get-text-property (point)
                                                    'dsh-emacs-markdown-frozen))
                            (not (dsh-emacs-markdown--in-avoid-range-p
         ...
         ((and (looking-at dsh-emacs-markdown--table-line-regexp)
               (not (get-text-property pos 'dsh-emacs-markdown-frozen)))
          (let ((table-start pos)
                (table-end nil)
                (row-count 0))
...
(defun dsh-emacs-markdown--parse-table-row (start end)
...
a `\\' escape, or (b) carries `dsh-emacs-markdown-frozen' — in which
...
(cl-defun dsh-emacs-markdown--table-display-width (&key str window)
...
pipes between rows."
  (if (and window
      ...
      (condition-case nil
          (let ((char-px (dsh-emacs-markdown--table-char-pixel-width window))
                (real-px (dsh-emacs-markdown--table-measure-string str window)))
            (ceiling (/ (float real-px) char-px)))
        (error (string-width str)))
    (string-width str)))

(cl-defun dsh-emacs-markdown--table-longest-word (&key str window)
  "Return display width of longest word in STR.
Uses `dsh-emacs-markdown--table-display-width' so non-ASCII words
  ...
  (if (or (null str) (string-empty-p str))
    ...
    (let ((words (split-string str "[ \t\n]+" t)))
      (if words
          (apply #'max
                 (mapcar (lambda (w)
                           (dsh-emacs-markdown--table-display-width
                            :str w :window window))
                         words))
        0))))

(defun dsh-emacs-markdown--table-total-width (widths)
...
(defun dsh-emacs-markdown--table-allocate-widths (natural-widths min-widths target)
  "Shrink NATURAL-WIDTHS proportionally to fit TARGET, respecting MIN-WIDTHS."
  (let* ((total (dsh-emacs-markdown--table-total-width natural-widths))
         (excess (- total target)))
    (if (<= excess 0)
        natural-widths
...
ratio (see `dsh-emacs-markdown--table-face-width-ratio') so wrap
...
overflow an N-cell column and push the right pipe out of line."
  (let* ((ch (seq-elt text pos))
    ...
    (if-let* ((face (and window
        ...
        (condition-case nil
            (* base (dsh-emacs-markdown--table-face-width-ratio
                     face window))
          (error base))
      base)))
  ...
  (if (and window
      ...
      (condition-case nil
          (let* ((char-px (dsh-emacs-markdown--table-char-pixel-width window))
                 ...
                 (content-px (dsh-emacs-markdown--table-measure-string str window))
              ...
              (let* ((full-spaces (floor (/ (float pad-px) char-px)))
                ...
                (concat str
                        ...
                        (if (> remaining-px 0)
                          ...
                          "")))))
        (error (dsh-emacs-markdown--pad-table-string-ascii :str str :width width)))
    (dsh-emacs-markdown--pad-table-string-ascii :str str :width width)))

(cl-defun dsh-emacs-markdown--pad-table-string-ascii (&key str width)
...
(defun dsh-emacs-markdown--make-table-separator-cell (width)
  "Return a separator-cell string of WIDTH dashes."
  (make-string width
               (if dsh-emacs-markdown-table-use-unicode-borders ?─ ?-)))

(defun dsh-emacs-markdown--render-table-separator-row (col-widths)
  "Build the rendered separator line for COL-WIDTHS."
  (let ((pipe (if dsh-emacs-markdown-table-use-unicode-borders "┼" "|"))
        (left (if dsh-emacs-markdown-table-use-unicode-borders "├" "|"))
        (right (if dsh-emacs-markdown-table-use-unicode-borders "┤" "|")))
    (concat
     (propertize left 'face 'dsh-emacs-markdown-table-border)
     (mapconcat
      (lambda (w)
        (propertize (dsh-emacs-markdown--make-table-separator-cell (+ w 2))
                    'face 'dsh-emacs-markdown-table-border))
      col-widths
      (propertize pipe 'face 'dsh-emacs-markdown-table-border))
     (propertize right 'face 'dsh-emacs-markdown-table-border))))

(cl-defun dsh-emacs-markdown--render-table-data-row (&key processed-cells col-widths row-face window)
...
WINDOW, when given, is forwarded to `dsh-emacs-markdown--pad-table-string'
for pixel-accurate padding of non-ASCII content.

Each cell on the first physical line of a wrapped row carries
`dsh-emacs-markdown-table-cell-start' on its leading padding char so
`dsh-emacs-markdown-table-next-cell' / `-previous-cell' can navigate
logical rows (skipping the visual continuation lines)."
  (let* ((pipe (if dsh-emacs-markdown-table-use-unicode-borders "│" "|"))
         (styled-pipe (propertize pipe 'face 'dsh-emacs-markdown-table-border))
         (wrapped (seq-mapn
                   (lambda (cell width)
                     (dsh-emacs-markdown--table-wrap-text
                      cell width window))
         ...
         (force-pixel-flags
          (mapcar (lambda (cell)
                    (or (not (string-match-p (rx bos (* ascii) eos) cell))
                        (dsh-emacs-markdown--text-has-face-p cell)))
                  processed-cells))
        ...
        (seq-mapn
         (lambda (cell-lines width force-pixel)
           (let* ((line (if (< line-idx (length cell-lines))
                          ...
                          ""))
                  (padded (concat " "
                                  (dsh-emacs-markdown--pad-table-string
             ...
             (when (and (zerop line-idx) (> (length padded) 1))
               (put-text-property 1 2 'dsh-emacs-markdown-table-cell-start t padded))
             (push padded parts)))
...
(cl-defun dsh-emacs-markdown--preprocess-table (&key rows window)
...
need it should use `dsh-emacs-markdown--table-min-widths'.

When WINDOW is given, cell widths are measured with
pixel-accurate `dsh-emacs-markdown--table-display-width' so columns
  ...
  (let ((widths nil)
    ...
    (dolist (row rows)
      (if (map-elt row :separator)
        ...
        (let ((cells (dsh-emacs-markdown--parse-table-row
...
(cl-defun dsh-emacs-markdown--table-min-widths (&key processed-rows window)
  "Return the minimum (longest-word) widths per column.
Called only when a table needs to be allocated narrower than its
natural total — see `dsh-emacs-markdown--render-table-source'."
  (let ((min-widths nil))
    (dolist (entry processed-rows)
      (let ((cells (cdr entry))
            (col 0))
        (dolist (processed cells)
          (let ((mw (dsh-emacs-markdown--table-longest-word
                     :str processed :window window)))
...
(defun dsh-emacs-markdown--render-table (table)
  "Render TABLE by replacing [:start, :end] with the rendered :source.

The rendered chars carry:
  - `dsh-emacs-markdown-frozen t' — so subsequent passes skip them.
  - `dsh-emacs-markdown-table-source SOURCE' — the original markdown
    source, stashed so a future `dsh-emacs-markdown-replace-markup'
...
rendered region from inheriting either of our two properties."
  (let* ((source (map-elt table :source))
         ...
         (carried (dsh-emacs-markdown--carry-properties table-start)))
    ...
    (let ((end (+ table-start (length rendered))))
      ...
      (add-text-properties
       table-start end
       `(dsh-emacs-markdown-frozen t
         dsh-emacs-markdown-table-source ,source
         rear-nonsticky (dsh-emacs-markdown-frozen
                         dsh-emacs-markdown-table-source))))))

(defun dsh-emacs-markdown--carry-properties (pos)
...
`dsh-emacs-markdown-frozen', `dsh-emacs-markdown-table-source',
...
(cl-defun dsh-emacs-markdown--render-table-source (&key source window)
  "Render SOURCE (markdown table text) to a propertized string.

SOURCE may carry text properties from earlier passes (bold faces
on cell content, `dsh-emacs-markdown-frozen' on inline-code bodies,
...
prone to a few-pixel drift on emoji-heavy tables."
  (with-temp-buffer
    ...
    (let* ((rows (dsh-emacs-markdown--collect-table-rows))
           ...
           (target-width (when dsh-emacs-markdown-table-wrap-columns
                           (floor (* (dsh-emacs-markdown--display-width)
                                     dsh-emacs-markdown-table-max-width-fraction))))
           (needs-allocation (and target-width
                                  (> (dsh-emacs-markdown--table-total-width
                                      natural-widths)
                                     target-width)))
           ...
           (col-widths (if needs-allocation
                           (dsh-emacs-markdown--table-allocate-widths
                            natural-widths
      ...
      (dolist (entry processed-rows)
        (let* ((row (car entry))
               ...
               (is-zebra (and dsh-emacs-markdown-table-zebra-stripe
                              (not is-header)
                              (not is-separator)
                              (= (mod data-row-num 2) 1)))
               (row-face (cond
                          (is-header 'dsh-emacs-markdown-table-header)
                          (is-zebra 'dsh-emacs-markdown-table-zebra))))
          (unless (or is-header is-separator)
            (setq data-row-num (1+ data-row-num)))
          (push (if is-separator
                    (dsh-emacs-markdown--render-table-separator-row col-widths)
                  (dsh-emacs-markdown--render-table-data-row
...
(defun dsh-emacs-markdown--collect-table-rows ()
      ...
      (while (and (not (eobp))
        ...
        (push `((:start . ,(point))
                ...
                (:separator . ,(looking-at
                                dsh-emacs-markdown--table-separator-regexp)))
              rows)
        (setq row-num (1+ row-num))
        (forward-line 1))
      (nreverse rows))))

(defun dsh-emacs-markdown--find-separator-row-num (rows)
...
(cl-defun dsh-emacs-markdown--style-tables (&key avoid-ranges)
...
inserted text carries `dsh-emacs-markdown-frozen' so subsequent calls
...
Honours `dsh-emacs-markdown-prettify-tables'.  Cell content is taken
...
rendering inside cells is provided for free."
  (when dsh-emacs-markdown-prettify-tables
    ;; Process tables in reverse so earlier positions stay valid as
    ;; each replacement shifts everything after it.
    (dolist (table (nreverse (dsh-emacs-markdown--find-tables
                              :avoid-ranges avoid-ranges)))
      (dsh-emacs-markdown--render-table table))))

(defun dsh-emacs-markdown-table-next-cell ()
...
on `2', and a fourth signals `No more cells left'."
  (interactive)
  (dsh-emacs-markdown-table--move-cell :forward))

(defun dsh-emacs-markdown-table-previous-cell ()
...
Inverse of `dsh-emacs-markdown-table-next-cell'."
  (interactive)
  (dsh-emacs-markdown-table--move-cell :backward))

(defun dsh-emacs-markdown-table--move-cell (direction)
...
there's no cell in that direction."
  (let* ((cells (dsh-emacs-markdown-table--cell-starts))
...
(defun dsh-emacs-markdown-table--cell-starts ()
  "Return a sorted list of cell-start positions in the table at point.
Returns nil when point isn't inside a rendered dsh-emacs-markdown
table.  Navigable cells are tagged by the renderer with the
`dsh-emacs-markdown-table-cell-start' text property, so separator rows
and continuation lines of wrapped rows are skipped automatically."
  (when-let* ((region (dsh-emacs-markdown-table--region-at-point)))
    (let ((positions nil))
      (save-excursion
        (save-restriction
          ...
          (while (let ((m (text-property-search-forward
                           'dsh-emacs-markdown-table-cell-start t t)))
                   (when m
                     (push (prop-match-beginning m) positions)
                     t)))))
      (nreverse positions))))

(defun dsh-emacs-markdown-table--region-at-point ()
  "Return (START . END) of the rendered table at point, or nil."
  (when (get-text-property (point) 'dsh-emacs-markdown-table-source)
    (cons (or (previous-single-property-change
               (1+ (point)) 'dsh-emacs-markdown-table-source nil (point-min))
              (point-min))
          (or (next-single-property-change
               (point) 'dsh-emacs-markdown-table-source nil (point-max))
              (point-max)))))

(defun dsh-emacs-markdown--apply-faces-from (propertized buffer-start)
...
(defun dsh-emacs-markdown--highlight-code (code lang)
...
(defun dsh-emacs-markdown--resolve-lang-mode (lang)
  "Resolve LANG string to a major mode symbol, or nil.
LANG is case-folded and trimmed; `dsh-emacs-markdown-language-mapping'
is consulted for aliases before the `-mode' suffix is appended."
  (when (and lang (not (string-empty-p (string-trim lang))))
    (let* ((normalized (downcase (string-trim lang)))
           (resolved (or (map-elt dsh-emacs-markdown-language-mapping
                                  normalized)
                         normalized))
...
(defun dsh-emacs-markdown--make-ret-binding-map (fun)
...
(defun dsh-emacs-markdown--open-link (url)
  "Open URL.  Use local navigation for file links, `browse-url' otherwise."
  (unless (dsh-emacs-markdown--open-local-link url)
    (browse-url url)))

(defun dsh-emacs-markdown--open-local-link (url)
  "Open URL as a local file link if possible.
Return non-nil if handled, nil otherwise."
  (when-let* ((parsed (dsh-emacs-markdown--parse-local-link url)))
...
(defun dsh-emacs-markdown--parse-local-link (url)
...
(defun dsh-emacs-markdown--resolve-image-url (url)
...
(defun dsh-emacs-markdown--image-max-width ()
  "Return the maximum image width in pixels.
Resolves `dsh-emacs-markdown-image-max-width' which may be an integer
(pixels) or a float between 0 and 1 (ratio of window body width)."
  (if (floatp dsh-emacs-markdown-image-max-width)
      (let ((window (or (get-buffer-window (current-buffer))
                        (frame-first-window))))
        (round (* dsh-emacs-markdown-image-max-width
...
(defun dsh-emacs-markdown--extending-table-start ()
...
- A line already carries `dsh-emacs-markdown-table-source' —
      ...
      (let (rendered-table-start
        ...
        (while continue
          (cond
           ...
           ((get-text-property (point) 'dsh-emacs-markdown-table-source)
           ...
           ;; mid-stream.
           ((looking-at dsh-emacs-markdown--table-pending-line-regexp)
...
- a rendered table that might still extend (see
  ...
  `dsh-emacs-markdown-table-source' and folds streamed rows in.
...
(defun dsh-emacs-markdown--sort-ranges (&rest range-collections)
...
(defun dsh-emacs-markdown--source-block-ranges ()
...
(defun dsh-emacs-markdown--frozen-ranges ()
  "Return ranges of buffer chars tagged `dsh-emacs-markdown-frozen'.
...
invoked many times as content grows."
  (let ((ranges '())
        ...
        (limit (point-max)))
    (while (< pos limit)
      (if (get-text-property pos 'dsh-emacs-markdown-frozen)
          (let ((end (or (next-single-property-change
                          pos 'dsh-emacs-markdown-frozen nil limit)
                         limit)))
...
(cl-defun dsh-emacs-markdown--inline-code-ranges (&key avoid-ranges)
        ...
        (let ((line-end (line-end-position))
          ...
          (while (re-search-forward "`" line-end t)
            (let ((pos (match-beginning 0)))
              (unless (dsh-emacs-markdown--in-avoid-range-p pos pos avoid-ranges)
                (if open
                    (progn
...
(defun dsh-emacs-markdown--deconstruct (text)
...
;;; dsh-emacs compatibility API

(defun dsh-emacs-markdown-render (text)
  "Convert assistant TEXT to rendered, propertized Markdown.
This is the dsh-emacs adapter around the in-place renderer ported from
`agent-shell-markdown'.  A final newline is supplied while converting so
streaming-safe constructs such as a final heading are complete; the caller
will add the display separator itself."
  (let ((source (or text "")))
    (dsh-emacs-markdown-convert
     (if (string-suffix-p "\n" source)
         source
       (concat source "\n")))))

(provide 'dsh-emacs-markdown)

;;; dsh-emacs-markdown.el ends here
