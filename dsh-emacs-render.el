...
the emoji in `dsh-emacs--variant-icons'.")
...
(defun dsh-emacs-render--tool-icon-svg (variant color)
  "Return the dsh-web SVG icon for VARIANT, filled with COLOR."
  (let ((template (cdr (assoc variant dsh-emacs--tool-icon-svgs))))
    (when template
      (replace-regexp-in-string "__C__" color template t t))))

(defun dsh-emacs-render--tool-icon (variant)
...
otherwise it falls back to the emoji glyph in `dsh-emacs--variant-icons'."
  (let ((glyph (or (cdr (assoc variant dsh-emacs--variant-icons)) "🔧")))
        ...
        (condition-case nil
            (let* ((fg (face-foreground 'dsh-emacs-tool-icon-face nil t))
              ...
              (propertize glyph
                          'display
                          (create-image
                           (dsh-emacs-render--tool-icon-svg variant color)
